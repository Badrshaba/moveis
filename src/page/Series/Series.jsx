...
import { backpage, firstpage, getAllSeriesPage, lastpage, nextpage } from '../../system/SeriesPageSlide';
...
const Series = () => {
  ...
  const {SeriesPage,loading,numPage}= useSelector(e=>e.SeriesPage)
  useEffect(()=>{
    dic(getAllSeriesPage(numPage))
  },[numPage])
  ...
  return (
    <div>
      <h1 className=' text-center text-info mt-3'>Series</h1>
...
<p className=' text-white d-flex justify-content-between'>Rating : <span className=' text-warning'><AiFillStar/><AiFillStar/><AiFillStar/><AiFillStar/><AiOutlineStar className=' text-black-50'/></span></p>
            ...
            >
                {e.overview}
                      ...
                      <div className=' w-100 '>
                      <Button as={Link} to={`/series/details/${e.id}`} variant="info">Details</Button>{' '}
                      </div>
